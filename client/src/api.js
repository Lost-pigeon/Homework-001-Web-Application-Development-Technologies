...
export async function createMaster(fio) {
  const res = await fetch(`${API_BASE}/masters`, {
    method: "POST",
...
export async function updateMaster(id, fio) {
...
export async function getTasksForMaster(masterId) {
  const res = await fetch(`${API_BASE}/masters/${masterId}/tasks`);
  return res.json();
}
export async function createTaskForMaster(masterId, address, complexity) {
  const res = await fetch(`${API_BASE}/masters/${masterId}/tasks`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
